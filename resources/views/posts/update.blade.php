@extends('layout')
@section('title', 'update_post')
@section('content')

    <main class="container mx-auto p-4">
        <h2 class="text-2xl font-semibold mb-4">آپدیت پست</h2>
        <form class="bg-white p-6 shadow rounded" action="{{ route('posts.update',$post->id) }}" method="POST">
            @csrf
            @method('put')
            <div class="mb-4 ">
                <label class="block text-gray-700">عنوان پست</label>
                <input type="text" name="title" class="w-full p-2 border rounded mt-1" value="{{ old('title',$post->title) }}" >
            </div>
            <div class="mb-4 ">
                <label class="block text-gray-700">محتوای پست</label>
                <textarea type="text" name="body" class="w-full p-2 border rounded mt-1" rows="4" >{{ old('body',$post->body) }}</textarea>
            </div>
            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded">آپدیت پست</button>
